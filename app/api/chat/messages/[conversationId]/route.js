...
export const GET = async (_req, { params }) => {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Error listing messages' }), {
      status: 500,
    });
  }
};
  ...
  try {
    ...
    if (recipients.length > 0) {
      ...
      if (Object.keys(incObj).length > 0) {
        await Conversation.updateOne({ _id: conversationId }, { $inc: incObj });
        ...
        for (const rid of recipients) {
          ...
          await pusherServer.trigger(`user-${rid}`, 'unread-updated', {
